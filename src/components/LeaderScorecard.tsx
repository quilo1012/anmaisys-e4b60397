...
export function LeaderScorecard({ leaderName, from, to, shift = "all" }: {
  ...
  const { data: actions = [], isError: eActions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const COLUMNS =
        "id, status, severity, recorded_at, labels, department, line, action_no, description, title, error_type, shift, validation_status, validated_at, validated_by, attachments, closed_at, domain, safety_kind, points_at_creation, scoring_version_id, classification";
