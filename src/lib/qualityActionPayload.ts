 ...
 */
/**
 * The grade a quality action is saved with — chosen on the form, never derived.
 *
 * It WAS derived, from 18/08 until change 2: the form stopped asking for a severity
 * because `actionPoints()` let the priced labels REPLACE the grade, which made the
 * field name a number the system did not use. Deriving it from the labels was the
 * correct response to that rule.
 *
 * MAX removed the rule and with it the reason. A grade derived from the label total is
 * the label total wearing a grade's name, so `MAX(labels, derived)` compares a number
 * with itself and change 2 would have been a no-op for every action this form creates.
 * The grade is a person's judgement about the deviation again; the labels price it; the
 * two are compared and the higher wins.
 *
 * Safety passes through unchanged and always did. A safety occurrence scores 0 whatever
 * it is graded, so its grade is a description rather than a price — and the safety form
 * classifies by `safety_kind`, which is what the weekly H&S scorecard counts.
 */
function qualitySeverity(form: QualityActionFormInput): string | null {
  return form.severity || null;
}
