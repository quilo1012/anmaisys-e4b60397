  ...
  const { data: ragTargets = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("rag_weekly_entries")
        .select("entry_date, shift, line, target_qty")
      ...
      const { data } = await q;
      return (data ?? []) as { entry_date: string; shift: string; line: string; target_qty: number | null }[];
    },
  });
  const ragTargetMap = useMemo(() => {
    const m = new Map<string, number>();
    for (const r of ragTargets) m.set(`${r.entry_date}|${r.shift}|${r.line}`, Number(r.target_qty ?? 0));
    return m;
  }, [ragTargets]);
