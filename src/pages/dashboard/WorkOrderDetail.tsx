    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardContent className="pt-6">
              ...
              <p className="font-medium">{wo.operator?.name || "—"}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-muted-foreground">Engineer</p>
              <p className="font-medium">{wo.engineer?.name || "—"}</p>
            </CardContent>
          </Card>
          {wo.closer?.name && (
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="font-medium">{wo.closer.name}</p>
              </CardContent>
            </Card>
          )}
        </div>
            ...
            {responseTime !== null && (
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-xl font-bold">{responseTime} min</p>
                </CardContent>
              </Card>
            )}
            {totalTime !== null && (
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {wo.status === "completed" && (
                <TimelineItem icon={CheckCircle} label="Completed" time={wo.completed_at} className="bg-green-100 text-green-700" />
              )}
              {wo.status === "force_closed" && (
                <TimelineItem icon={XCircle} label="Force Closed" time={wo.completed_at} className="bg-gray-100 text-gray-700" />
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-muted-foreground text-sm">Parts tracking will be available in Phase 3.</p>
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
