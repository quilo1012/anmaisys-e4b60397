-- ================================================================
-- APLICAR TUDO, POR ORDEM — um só ficheiro
-- ================================================================
-- Os nove blocos de docs/apply/, concatenados na ordem em que os seus carimbos
-- mandam: 01, 02, 03, 04, 05, 05b, 06, 07, 08.
--
-- PORQUE EXISTE: o modo de falha desta aplicação nunca foi o SQL — foi correr o
-- bloco 6 antes do 5, ou esquecer o 05b, que não está no pending-migrations-apply.sql.
-- Com um ficheiro não há ordem para acertar.
--
-- COMO CORRER: SQL Editor do projecto ybtrzqzliepknpzqdajx (a base da app), com um
-- papel que ignora o RLS — o `postgres` do editor serve. Cole tudo e corra.
--
-- NÃO gasta créditos do Lovable. O Caminho A (pedir ao agente que corra o SQL) gasta,
-- e o auto top-up está em falha.
--
-- SE FALHAR: cole tudo de novo depois de tratar a causa. NÃO cole por pedaços.
--
-- Porquê: o SQL Editor manda o script como uma consulta só, e o Postgres embrulha-a
-- numa transacção implícita. Não há aqui `ALTER TYPE … ADD VALUE` nem
-- `CREATE INDEX CONCURRENTLY` — as duas coisas que quebrariam isso. Portanto isto é
-- atómico: se falhar na linha 2100, as 2099 anteriores são desfeitas e a base fica
-- exactamente como estava. Uma falha custa-lhe uma mensagem de erro, não meia migração.
--
-- *** MAS NÃO COLE DUAS VEZES DEPOIS DE UMA PASSAGEM QUE TENHA CORRIDO ATÉ AO FIM. ***
--
-- Correr uma vez numa base limpa está revisto e está bem: nada é referido antes de
-- existir. Correr por cima de uma aplicação JÁ BEM SUCEDIDA é que rebenta, em dois
-- sítios conhecidos:
--
--   · linha ~280  DROP VIEW IF EXISTS public.v_leader_weekly_scorecard; — sem CASCADE.
--                 Já existiriam v_leader_weekly_scorecard_periods, v_scorecard_trend_leader
--                 e _line a depender dela: 2BP01, cannot drop view … other objects depend.
--
--   · linhas ~418-453  o backfill lê line_leader, ccp_check_completed e mais três colunas
--                 que as linhas ~459-463 largam. Na segunda vez o CREATE TABLE IF NOT
--                 EXISTS é saltado, as colunas já não existem, e o UPDATE nem chega a ser
--                 analisado: ERROR: column "ccp_check_completed" does not exist.
--
-- Para saber em que caso está, corra o verificador antes de repetir seja o que for.
--
-- Notas de ambiente, verificadas: a linha ~502 usa NULLS NOT DISTINCT, que exige
-- PostgreSQL 15+ (a base corre 17). E o `SET LOCAL search_path` a seguir é a ÚNICA linha
-- de SQL que não vem dos nove blocos — está lá porque o btree_gist é instalado no schema
-- `extensions` e os constraints de exclusão das linhas ~181 e ~220 precisam de o ver pelo
-- nome; sem isso o Postgres diz "data type uuid has no default operator class for access
-- method gist" e o `EXCEPTION WHEN duplicate_object` à volta NÃO apanha esse erro.
SET LOCAL search_path = public, extensions;
--
-- DEPOIS: corra supabase/tests/verify_scorecard_v2_deployment.sql (só lê). Tem de dar
-- PRESENTE em tudo, incluindo v_leader_weekly_scorecard.volume_source.
--
-- E DEPOIS DISSO: apague src/lib/optionalDomain.ts e o fallback nos quatro ecrãs —
-- existe só porque a coluna `domain` não existia, e passa a esconder deriva a sério.
-- ================================================================
...
-- ################################################################
-- BLOCO 01
-- 01-20260815120000_a_label_can_carry_its_own_points.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 02
-- 02-20260815140000_health_and_safety_is_the_second_gate.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 03
-- 03-20260816090000_the_screen_asks_the_database.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 04
-- 04-20260817090000_safety_shares_the_log_but_not_the_score.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 05
-- 05-20260817093000_the_week_counts_its_own_safety.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 05b
-- 05b-20260817120000_safety_has_its_own_labels.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 06
-- 06-20260818090000_a_gate_is_a_ceiling_not_a_weight.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 07
-- 07-20260819090000_the_score_crosses_the_board_rpc.sql
-- ################################################################
...
-- ################################################################
-- BLOCO 08
-- 08-20260820090000_filling_a_week_is_not_approving_it.sql
-- ################################################################
