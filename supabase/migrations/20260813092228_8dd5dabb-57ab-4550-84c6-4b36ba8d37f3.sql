CREATE TABLE public.downtime_corrections (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  downtime_event_id uuid NOT NULL REFERENCES public.downtime_events(id) ON DELETE CASCADE,
...
GRANT SELECT ON public.downtime_corrections TO authenticated;
GRANT ALL ON public.downtime_corrections TO service_role;

ALTER TABLE public.downtime_corrections ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Scoped downtime_corrections select"
ON public.downtime_corrections
FOR SELECT
TO authenticated
USING (
  ...
  OR EXISTS (
    ...
    WHERE wo.id = downtime_corrections.work_order_id
      AND (
...
CREATE INDEX idx_downtime_corrections_wo ON public.downtime_corrections(work_order_id, corrected_at);
CREATE INDEX idx_downtime_corrections_event ON public.downtime_corrections(downtime_event_id);
