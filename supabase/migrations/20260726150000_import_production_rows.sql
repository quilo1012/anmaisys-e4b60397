-- Transactional Production import. The client used to loop inserts row-by-row,
-- so a failure mid-way left a partial import (some sessions created, others not)
-- with no rollback. This runs the whole import in one function = one transaction:
-- if anything fails, everything rolls back.
--
-- Matching mirrors the client: case-insensitive + base code (ignoring a "- Bn"
-- batch suffix); anything unmatched is kept as a free-text SKU (sku_code_text)
-- rather than dropped.
...
BEGIN
  IF _uid IS NULL OR NOT (
    has_role(_uid,'admin'::app_role) OR has_role(_uid,'manager'::app_role) OR has_role(_uid,'supervisor'::app_role)
  ) THEN
  ...
  LOOP
    ...
    END IF;
    -- one loop iteration == one distinct (date, shift, line) == one session
    _sessions := _sessions + 1;

    -- exact (case-insensitive), then base code (drop a "- Bn" suffix)
...
GRANT EXECUTE ON FUNCTION public.import_production_rows(jsonb) TO authenticated;
