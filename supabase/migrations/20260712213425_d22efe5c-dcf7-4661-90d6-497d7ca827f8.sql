...
ALTER TABLE public.leader_pins ADD COLUMN IF NOT EXISTS line text;

DROP FUNCTION IF EXISTS public.list_leaders();
CREATE OR REPLACE FUNCTION public.list_leaders()
RETURNS TABLE(id uuid, name text, is_active boolean, line text, created_at timestamptz)
LANGUAGE sql SECURITY DEFINER SET search_path = public
AS $$ SELECT l.id, l.name, l.is_active, l.line, l.created_at FROM public.leader_pins l ORDER BY l.name; $$;
...
CREATE OR REPLACE FUNCTION public.create_leader(_name text, _pin text, _line text DEFAULT NULL)
...
BEGIN
  ...
  END IF;
  INSERT INTO public.leader_pins(name, pin_hash, line)
  VALUES (trim(_name), extensions.crypt(_pin, extensions.gen_salt('bf', 10)), NULLIF(trim(_line),''))
...
CREATE OR REPLACE FUNCTION public.update_leader(
  ...
  _pin text DEFAULT NULL, _line text DEFAULT NULL
...
BEGIN
  ...
  IF _line IS NOT NULL THEN
    UPDATE public.leader_pins SET line = NULLIF(trim(_line),'') WHERE id = _id;
  END IF;
END $$;
...
BEGIN
  ...
  SELECT l.id, l.name, l.line INTO _leader FROM public.leader_pins l
  ...
  IF _leader.id IS NOT NULL THEN
    ...
    RETURN jsonb_build_object('success', true, 'engineer_id', _leader.id,
      'engineer_name', _leader.name, 'is_leader', true, 'leader_line', _leader.line);
  END IF;
