...
-- "trigger" field the schema never had, so every call since the schema was
...
--
-- Result: no production sync, and intouch-sync-actuals-5min burning ~288 failed
-- calls a day against the iTouching rate limit. Confirmed live: net._http_response
-- showed {"error":{}} / 400 recurring right up to the fix.
--
-- (Supersedes the closed PR #132; applied directly to the live DB via
-- cron.alter_job since Lovable does not deploy migrations. Kept here for record.)
...
-- every two minutes), which tripped iTouching's "100 calls per minute" limit on
-- 2026-07-04. One is enough — keep the 60s job, drop the 120s.
