-- Change reopen_wo_as_recurrence to REOPEN the same WO instead of creating a new one.
-- The same WO number is preserved; a new episode is added so times accumulate.
...
BEGIN
  ...
  SELECT id, wo_number, requester_name, machine, description, priority,
         ...
         engineer_id, engineer_name, current_episode
    INTO _orig
  ...
  -- Close the previous episode if still open
  ...
  -- Reopen SAME WO: clear close timestamps, bump reopen_count, restore engineer link
  ...
  -- Open a new downtime event (line is stopped again because problem returned)
  ...
  RETURN jsonb_build_object(
    ...
    'reopen_count', _orig.current_episode  -- previous episode count
  );
END;
$function$;