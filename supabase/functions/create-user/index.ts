...
const createUserSchema = z.object({
  ...
  role: z.enum(["admin", "manager", "supervisor", "quality_supervisor", "maintenance_manager", "planner", "engineer", "co_engineer", "operator", "viewer", "warehouse", "production_office_admin"], { errorMap: () => ({ message: "Invalid role" }) }),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Only admins can create elevated/read-only staff roles
    if ((role === "admin" || role === "manager" || role === "supervisor" || role === "quality_supervisor" || role === "maintenance_manager" || role === "planner" || role === "viewer" || role === "production_office_admin") && !isAdmin) {
      return jsonResponse({ error: "Only admins can assign Admin, Manager, Supervisor, Quality Supervisor, Maintenance Manager, Planner, Viewer or Production Office Admin roles" }, 403);
    }
